...
func (d *dlnaServer) connectionManagerSCPD(w http.ResponseWriter, _ *http.Request) {
...
func (d *dlnaServer) control(w http.ResponseWriter, r *http.Request) {
	...
	action := actionName(r.Header.Get("SOAPAction"), env.Body.Action)
	switch action {
	case "Browse":
		var in browseInput
		if err := xml.Unmarshal(env.Body.Action, &in); err != nil {
			soapFault(w, "InvalidArgs")
			return
		}
		result, n, total := d.browse(r.Context(), in.ObjectID, in.BrowseFlag, in.StartingIndex, in.RequestedCount)
		soapBrowse(w, result, n, total)
...
func (d *dlnaServer) connectionManagerControl(w http.ResponseWriter, _ *http.Request) {
	soapActionResponse(w, "GetProtocolInfoResponse", `<Source>http-get:*:video/mp4:*</Source><Sink></Sink>`)
}
...
func (d *dlnaServer) browse(ctx context.Context, objectID, flag string, start, requested uint32) (string, uint32, uint32) {
	...
	if objectID != "0" && objectID != "" {
		...
		currentRel = filepath.ToSlash(strings.TrimPrefix(rel, "/"))
		root = filepath.Join(d.library.Root, filepath.FromSlash(currentRel))
	}
	items, err := os.ReadDir(root)
	if err != nil {
		return didlList(""), 0, 0
	}
	entries := make([]didlObject, 0, len(items))
	for _, e := range items {
		...
		if e.IsDir() {
			rel := filepath.ToSlash(filepath.Join(currentRel, e.Name()))
	...
	if flag == "BrowseMetadata" {
		if objectID == "0" || objectID == "" {
			return didlList(didlContainer(d.name, "0", "-1")), 1, 1
		}
		for _, x := range entries {
			if x.ID == objectID {
				return didlList(x.toDIDL(d.baseURL())), 1, 1
			}
		}
		return didlList(""), 0, 0
	}
	if start >= uint32(len(entries)) {
		return didlList(""), 0, uint32(len(entries))
	}
	...
	for _, x := range entries[start:end] {
		b.WriteString(x.toDIDL(d.baseURL()))
	}
	return didlList(b.String()), uint32(end - start), uint32(len(entries))
}
...
func (d *dlnaServer) media(w http.ResponseWriter, r *http.Request) {
	...
	full := filepath.Join(d.library.Root, clean)
	root, _ := filepath.Abs(d.library.Root)
...
func (d *dlnaServer) start(ctx context.Context, _ func(string)) {
...
func (d *dlnaServer) ssdp(ctx context.Context, base string) {
	g := net.ParseIP("239.255.255.250")
	conn, err := net.ListenMulticastUDP("udp4", nil, &net.UDPAddr{IP: g, Port: 1900})
	if err != nil {
		return
	}
	defer conn.Close()
	...
	announce := func() {
		...
		_, _ = conn.WriteToUDP([]byte(msg), &net.UDPAddr{IP: g, Port: 1900})
	}
	announce()
	buf := make([]byte, 4096)
	...
	for {
		...
		if err == nil && strings.Contains(strings.ToLower(string(buf[:n])), "m-search") {
			resp := fmt.Sprintf("HTTP/1.1 200 OK\r\nCACHE-CONTROL: max-age=1800\r\nDATE: %s\r\nEXT:\r\nLOCATION: %s\r\nSERVER: Hermit/1.0 UPnP/1.0\r\nST: urn:schemas-upnp-org:device:MediaServer:1\r\nUSN: %s\r\n\r\n", time.Now().UTC().Format(http.TimeFormat), loc, usn)
...
func httpXML(w http.ResponseWriter, body string) {
...
func (x didlObject) toDIDL(base string) string {
	if x.Container {
...
func didlList(content string) string {
	return `<?xml version="1.0" encoding="utf-8"?><DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/">` + content + `</DIDL-Lite>`
}
...
func actionName(header string, raw []byte) string {
	if i := strings.Index(header, "#"); i >= 0 {
		return strings.Trim(header[i+1:], `"'`)
	}
	s := string(raw)
	a := strings.Index(s, "<")
	if a < 0 {
	...
	n := s[:b]
	if c := strings.LastIndex(n, ":"); c >= 0 {
		n = n[c+1:]
	}
	return n
}
func soapBrowse(w http.ResponseWriter, result string, returned, total uint32) {
	soapActionResponse(w, "BrowseResponse", fmt.Sprintf(`<Result>%s</Result><NumberReturned>%d</NumberReturned><TotalMatches>%d</TotalMatches><UpdateID>1</UpdateID>`, xmlEscapeText(result), returned, total))
}
func soapActionResponse(w http.ResponseWriter, action, body string) {
	w.Header().Set("Content-Type", `text/xml; charset="utf-8"`)
	_, _ = fmt.Fprintf(w, `<?xml version="1.0"?><s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"><s:Body><u:%s xmlns:u="urn:schemas-upnp-org:service:ContentDirectory:1">%s</u:%s></s:Body></s:Envelope>`, action, body, action)
}
func soapFault(w http.ResponseWriter, code string) {
	w.Header().Set("Content-Type", `text/xml; charset="utf-8"`)
	w.WriteHeader(http.StatusInternalServerError)
	_, _ = fmt.Fprintf(w, `<?xml version="1.0"?><s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/"><s:Body><s:Fault><faultcode>s:Client</faultcode><faultstring>%s</faultstring></s:Fault></s:Body></s:Envelope>`, html.EscapeString(code))
}
